...
class App:
    def __init__(self, root):
        ...
        # Target Window
        ttk.Label(root, text="Target Window Title:").grid(row=2, column=0, padx=5, pady=5, sticky="e")
        self.window_title_entry = ttk.Entry(root, width=40)
        self.window_title_entry.grid(row=2, column=1, padx=5, pady=5)

        # Toggle Key
        ttk.Label(root, text="Toggle Key:").grid(row=3, column=0, padx=5, pady=5, sticky="e")
        ...
        self.fkey_combo.grid(row=3, column=1, padx=5, pady=5, sticky="w")
    ...
    def start_typing(self):
        def run():
            ...
            if not message or not window_title:
                messagebox.showerror("Missing Input", "Message and Window Title cannot be empty.")
                self.running = False
                return

            while self.running:
                self.simulate_typing_and_enter(message, window_title)
                time.sleep(timer)

        threading.Thread(target=run, daemon=True).start()

    def simulate_typing_and_enter(self, message, target_window_title):
        try:
            windows = gw.getWindowsWithTitle(target_window_title)
            if not windows:
                raise Exception("No matching window found")

            win = windows[0]
            win.activate()
            time.sleep(0.3)  # Let the window come to front
            ...
            messagebox.showerror("Window Error", f"Could not find or activate window titled:\n\n'{target_window_title}'")
